...
export class RoleService {
  ...
  async getRoles(): Promise<ApiResponse<Role[]>> {
    ...
    const roleData: Role[] = roles.map(role => ({
      id: role.id,
      name: role.name,
  ...
  async createRole(roleData: Omit<Role, 'id'>): Promise<ApiResponse<Role>> {
    ...
    const responseData: Role = {
      ...
      image: savedRole.image,
      language: savedRole.language
    };
  ...
  async updateRole(id: number, roleData: Partial<Omit<Role, 'id'>>): Promise<ApiResponse<Role>> {
    const role = await this.roleRepository.findOne({
      where: { id, isActive: true }
    });
    ...
    const responseData: Role = {
      ...
      image: updatedRole.image,
      language: updatedRole.language
    };
